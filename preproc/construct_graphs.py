...
def ProcNodes(nodesData, nodeDict, model, tokenizer):
# def ProcNodes(nodesData, nodeDict):
    ...
    #     nodeType.append(nodeData[1])

    # 将节点数据划分为批次
    batch_size = 32  # 您可以根据需要调整批次大小
    node_batches = [nodesDataNew[i:i + batch_size] for i in range(0, len(nodesDataNew), batch_size)]
    ...
    #     # 收集当前批次的所有节点内容
    #     # batch_tokens = []
    #     for nodeData in batch:
    #         nodeContent = nodeData[6]
    #         nodeVec = encoder(nodeContent).numpy().squeeze(0)
    #         nodeData = extract_graphs.ProcNode(nodeData, 'PatchCPG')
    #         nodeEmbed = GetNodeEmbedding(nodeData)
    #         # nodeAttr.append(nodeVec)
    #         nodeAttr.append(np.concatenate((nodeVec, nodeEmbed), axis=0))
    #         nodeType.append(nodeData[1])
    ...
    for batch in node_batches:
        ...
        for nodeData in batch:
            nodeContent = nodeData[6]
            code_tokens = tokenizer.tokenize(str(nodeContent))[:_MaxLen_ - 2]
            source_tokens = [tokenizer.cls_token] + code_tokens + [tokenizer.sep_token]
            batch_source_tokens.append(source_tokens)

        # 批量转换为ID
        max_length = max(len(tokens) for tokens in batch_source_tokens)
        batch_source_ids = []
        batch_source_masks = []
        for tokens in batch_source_tokens:
            ...
            source_mask = [1] * len(tokens) + [0] * padding_length
            batch_source_masks.append(source_mask)

        # 转换为Tensor
        batch_source_ids = torch.tensor(batch_source_ids).to(device)
        batch_source_masks = torch.tensor(batch_source_masks).to(device)

        # 进行批量推理
        with torch.no_grad():
            batch_node_embed = model(
                embed=True,
                input_ids=batch_source_ids,
                attention_mask=batch_source_masks
            )

        # 将嵌入添加到结果列表中
        for i, nodeData in enumerate(batch):
            nodeEmbed = batch_node_embed[i]  # 获取当前节点的嵌入
            nodeAttr.append(np.array(nodeEmbed.tolist()))
            nodeType.append(nodeData[1])
    ...
    return nodeAttr, nodeType, 0

class ContentEncoder(object):
    def __init__(self, device=None):
        self.device = device
        self.model = KeyedVectors.load("models/word2vec.wordvectors", mmap='r')
