...
def build_load_model(load_model=True):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    config_class, model_class, tokenizer_class = MODEL_CLASSES["codet5_CC"]

    config = config_class.from_pretrained("./models/codet5")
     # default codet5 tokenizer
    tokenizer_name = "./models/codet5"

    tokenizer = tokenizer_class.from_pretrained(tokenizer_name)

    # args.model_name_or_path = "t5-base"

    model = model_class.from_pretrained("./models/codet5")
    # model = model_class(config)

    tokenizer, config = enrich_vocab(tokenizer, config)
    ...
    model.to(device)
    # 载入CCT5
    # model.load_state_dict(torch.load("./models/pytorch_model.bin", map_location="cpu"))
...
class CodeChangeModel(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if "embed" in kwargs:   # for embedding
